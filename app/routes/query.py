...
def query(
    ...
    w_vec: float = Query(0.7, description="벡터 유사도 가중치"),
    w_lex: float = Query(0.3, description="lexical(trigram) 유사도 가중치"),
    diversity_penalty: float = Query(0.9, description="다양성 페널티 계수"),
    ...
    if group_id:
        try:
            ...
            raise HTTPException(status_code=422, detail="group_id는 UUID 형식이어야 합니다.")
